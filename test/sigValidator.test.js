...
describe("Test Sig Validator contract", async () => {
  ...
  it("test query Sig", async () => {
    const querySig = async (issuerIndex, holderIndex, claim, query) => {
      const issuer = users[issuerIndex];
      const holder = users[holderIndex];
      const kycSigInput = await zidenjs.querySig.kycGenerateQuerySigInput(
        issuer.auths[0].privateKey,
        issuer.auths[0].value,
        claim,
        issuer.state
      );
      const kycNonRevInput =
        await zidenjs.querySig.kycGenerateNonRevQuerySigInput(
          claim.getRevocationNonce(),
          issuer.state
        );
      const inputs =
        await zidenjs.querySig.holderGenerateQuerySigWitnessWithPrivateKey(
          claim,
      ...
      const { proof, publicSignals } = await snarkjs.groth16.fullProve(
        inputs,
        "build/credentialAtomicQuerySig.wasm",
        "build/credentialAtomicQuerySig.zkey"
      );
      const { a, b, c, public } = await callData(proof, publicSignals);
      const tx = await testContract.verifySig(a, b, c, public);
      await tx.wait();
    };

    await querySig(0, 3, claim0, query0);
    await querySig(1, 4, claim1, query1);
    await querySig(2, 5, claim2, query2);

    try {
      await querySig(2, 5, expiredClaim, query2);
    } catch (err) {
      console.log(err);
    }
  });
